...
(() => {
    ...
    let UrRule;
    (function (UrRule) {
        UrRule["FINKEL"] = "finkel";
        UrRule["MASTERS"] = "masters";
        UrRule["BLITZ"] = "blitz";
        UrRule["CUSTOM"] = "custom";
    })(UrRule || (UrRule = {}));
    let UrBot;
    (function (UrBot) {
        UrBot["EASY"] = "easy";
     ...
     */
    function injectUI() {
        if (document.getElementById(UI_CONSTANTS.PANEL_ID))
            return;
        ...
        const ruleSelect = document.getElementById(UI_CONSTANTS.RULE_SELECT_ID);
        const botSelect = document.getElementById(UI_CONSTANTS.BOT_SELECT_ID);
        const startBtn = document.getElementById(UI_CONSTANTS.START_BTN_ID);
        // Load saved settings or defaults
        chrome.storage.local.get(['rules', 'bot'], (result) => {
            const urlParams = new URLSearchParams(window.location.search);
            const currentRule = urlParams.get('rules') || result.rules || UrRule.FINKEL;
            const currentBot = urlParams.get('bot') || result.bot || UrBot.HARD;
